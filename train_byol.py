"""
BYOL Training Script.

Usage:
    python train_byol.py

BYOL - SSL метод с EMA target network.
Более стабильный чем SimSiam, но требует больше памяти (две копии модели).
"""
...
from src.system_byol import BYOLSystem
...
@hydra.main(config_path="configs", config_name="train_byol_config", version_base="1.3")
def main(cfg: DictConfig):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='byol-{epoch:02d}-{train_byol_loss:.4f}',
        monitor='val_byol_loss',
        save_top_k=1,
        mode='min',
        save_last=True
    )
